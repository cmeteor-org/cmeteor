...
var CodeMirror = (function() {
  ...
  var IMG_ARR = ['.jpg', '.jpeg', '.gif', '.png', '.bmp'];
    ...
    if (files && files.length && window.FileReader && window.File) {
      if (/text/.test(files[0].type)){
        ...
        for (var i = 0; i < n; ++i) loadFile(files[i], i);
         ...
         _.extend(toastInfo, {icon: 'error', heading: '错误', text: '不支持非文本文件上传'});
         $.toast(toastInfo);
      }
    } else {
...
var toolbar = [
  ...
  {name: 'tag', action: selfTag, className: "fa fa-tag"},
  '|',
...
Editor.markdown = function(text) {
  if (window.marked || window.markdowniter) {
    ...
    return marked(text, markedOptions); /*|| markdowniter.render(text)*/
  }
};
